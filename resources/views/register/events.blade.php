...
@section('title', 'Pilih kode acara')
@section('meta_description', 'Pilih kode acara dan isi catatan waktu untuk '.$competition->name)
...
@section('content')
    <div class="mx-auto max-w-3xl pb-36 sm:pb-0">
        ...
        <p class="mt-1 text-sm leading-6 text-slate-600">
            Centang kode acara yang diikuti. Hanya nomor yang cocok dengan L/P dan kelompok umur peserta yang tampil di sini.
        </p>
        ...
        <div class="mt-5 rounded-2xl border border-slate-200 bg-white p-4 text-sm shadow-sm">
            <dl class="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <dd class="mt-0.5 font-medium">{{ $athlete->gender->value }} · {{ $athlete->birth_year }} → {{ $ageGroup->name }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500">Klub/sekolah</dt>
                    <dd class="mt-0.5 font-medium">{{ $athlete->club->name }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500">Kabupaten/kota</dt>
                    <dd class="mt-0.5 font-medium">{{ $athlete->club->city ?: '—' }}</dd>
                </div>
            </dl>
            <a href="{{ route('register.create', $competition) }}" class="mt-3 inline-block min-h-11 py-2 text-sm font-medium text-teal-800 hover:underline">Ubah data peserta</a>
        </div>
        ...
        <form method="POST" action="{{ route('register.events.store', $competition) }}" class="mt-6" id="event-form"
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <h2 class="text-base font-semibold text-slate-900">Kode acara untuk {{ $ageGroup->name }}</h2>
                <p class="mt-1 text-sm text-slate-600">Maksimal {{ $competition->max_events_per_athlete }} kode acara per peserta di kejuaraan ini.</p>
                ...
                <ul class="mt-4 space-y-3">
                    @forelse ($events as $event)
                        ...
                        <li data-event-card class="rounded-2xl border border-slate-200 bg-slate-50/50 p-4 {{ $checked ? 'selected' : '' }}">
                            <label for="event-{{ $event->id }}" class="flex cursor-pointer items-start gap-3">
                                ...
                                <span class="min-w-0 flex-1">
                                    <span class="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                                        <span class="font-mono text-lg font-bold text-teal-800"><span class="sr-only">Kode acara </span>{{ $event->paddedEventNumber() }}</span>
                            ...
                            <div data-seed-field class="mt-4 border-t border-teal-100 pt-3" @if (! $checked) hidden @endif>
                                <label for="seed-{{ $event->id }}" class="block text-sm font-medium text-slate-800">Catatan waktu</label>
                    ...
                    @empty
                        <li class="rounded-2xl border border-dashed border-slate-300 px-4 py-10 text-center text-sm text-slate-500">
                            Tidak ada kode acara yang boleh diikuti kelompok umur ini.
                        </li>
                    @endforelse
                </ul>
            </div>
            ...
            <div class="mt-4 hidden items-center justify-between gap-3 sm:flex">
                <p class="text-sm font-medium text-slate-700" id="quota">
                    Terpakai {{ $used }} dari {{ $competition->max_events_per_athlete }} kode acara yang diizinkan.
                </p>
